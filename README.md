...
Client / LLM
  └─ MCP: /mcp
      ├─ smart_research -> URL 自动抓取；关键词自动并用多源搜索，再让 Grok 汇总
      ├─ smart_fetch    -> Tavily Extract -> Firecrawl Scrape -> HTML fetch
      ├─ web_search     -> Grok / OpenAI-compatible AI 搜索回答
      ├─ web_fetch/map  -> Tavily REST 或第三方 Tavily MCP/Hikari
      ├─ libre_search   -> LibreSearch/SearXNG JSON 结构化搜索
      └─ search2api_chat -> Search-2api/search.sh 答案搜索
```

相比原始 Grok Search MCP 的 `Grok 搜索 + Tavily 抓取/映射 + Firecrawl 托底`，FusionSearch 多了两层可独立验证的搜索源：LibreSearch 提供结构化网页结果，Search-2api 提供 search.sh 风格答案。智能工具会尽量交叉取证；单个上游失效时，仍返回已经拿到的证据和明确错误。
...
- `smart_research`：推荐主入口。输入 URL 时自动抓取网页；输入关键词/问题时并用 LibreSearch、Search-2api、Tavily，再交给 Grok 汇总。Grok 失败时仍返回已取得证据。
- `smart_fetch`：推荐网页抓取入口。按 `Tavily Extract -> Firecrawl Scrape -> HTML fetch` 顺序降级，可选让 Grok 基于正文总结。
- `fusion_status`：返回五个核心模块的最近健康状态和脱敏 Key 状态。
