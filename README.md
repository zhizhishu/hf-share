...
Provider 工具仍然保留给显式使用：`web_search`、`web_fetch`、`web_map`、`libre_search`、`search2api_chat` 和 `fusion_research` 可用于调试、指定上游、复现某个 provider 行为，或让高级客户端自己编排证据。
...
推荐客户端优先使用三类智能工具，它们对应 Fusion Orchestrator 的默认证据流水线：

- `smart_research`：推荐主入口。Intent Router 会先判断输入是 URL 还是关键词/问题；URL 进入抓取流水线，关键词/问题进入多源研究流水线，并尽量交叉取证后再合成答案。
- `smart_fetch`：推荐网页抓取入口。按 `Tavily Extract -> Firecrawl Scrape -> HTML fetch` 顺序降级，归一化正文证据后可选让 Grok 基于抓取内容总结。
- `fusion_status`：推荐状态入口。返回 LibreSearch、Search-2api、Grok、Tavily、Firecrawl 的最近健康状态和脱敏 Key 状态。

Provider 工具仍可显式调用，适合调试上游、固定某个 provider、或由高级客户端自行编排证据：
...
- `fusion_research`：LibreSearch + Search-2api 取证，再交给 Grok 汇总；保留用于显式研究流程和兼容已有客户端。
